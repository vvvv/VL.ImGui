...
namespace VL.ImGui.Widgets.Primitives
{
    ...
    internal partial class Line : PrimitiveWidget
    {
        ...
        protected override void Draw(Context context, in ImDrawListPtr drawList, in System.Numerics.Vector2 offset)
        {
            ...
            //    drawList.AddPolyline(ref x, vectors.Length, color, IsClosed ? ImDrawFlags.Closed : ImDrawFlags.None, Thickness);
            //}
            //else
            {
                for (int i = 0; i < points.Count - 1; i++)
                {
                    drawList.AddLine(points[i].ToImGui() + offset, points[i + 1].ToImGui() + offset, color, Thickness);
                }

                if (IsClosed && points.Count > 1)
                {
                    drawList.AddLine(points[points.Count - 1].ToImGui() + offset, points[0].ToImGui() + offset, color, Thickness);
                }
            }
        }
    }
